...
class RoleCog(Cog, name="RoleManager"):
    ...
    async def create_team(self, ctx: Context, *name: str):
        ...
        team_category: CategoryChannel = await self.bot.guilds[
            0].create_category(
    ...
    async def give_author_role(self, ctx: Context,
        ...
        if role in ctx.author.roles:
            ...
            await ctx.channel.send(
                "Etwas ist falsch gelaufen und du hast die Rolle nicht bekommen. \nVersuchs nochmal!")
        else:
            await ctx.channel.send(
                "Irgendwas ist seehr falsch gelaufen. Bitte kontaktiere einen Dev.")
    ...
    async def remove_author_role(self, ctx: Context,
        ...
        if role not in ctx.author.roles:
            ...
            await ctx.channel.send(
                "Etwas ist falsch gelaufen und du hast die Rolle immer noch. \nVersuchs nochmal!")
        else:
            await ctx.channel.send(
                "Irgendwas ist seehr falsch gelaufen. Bitte kontaktiere einen Dev.")
    ...
    async def check_for_team_in_db(self, ctx: Context,
        ...
        team_name = " ".join(name)
        team = User.get_users(team="".join(VALID_CHARS.findall(team_name)))
